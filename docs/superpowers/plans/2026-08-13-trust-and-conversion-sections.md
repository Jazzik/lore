...
## Ruling 2026-08-13: демо-данные разрешены

Заказчик явным решением снял блокировку по недостающим данным: сайт ещё не опубликован, находится в активной разработке, и на этом этапе нужен наглядный прототип, а не финальные факты. Поэтому **все кейсы, компании, имена, цифры экономики, состав команды и параметры производства выдумываются** — вымышленные бренды несуществующих компаний, правдоподобные, но не настоящие.

Обязательные условия, компенсирующие выдумку:

1. Каждый файл и каждый namespace с выдуманными данными начинается с комментария-маркера:
   `// ДЕМО-ДАННЫЕ. Вымышлено для превью, не публиковать без замены на реальные.`
   Для JSON — ключ `"_demo": "вымышленные данные, заменить перед публикацией"` первым в namespace, если namespace целиком демонстрационный.
2. Названия брендов и авторов должны быть очевидно вымышленными и не совпадать с реальными существующими блогерами или компаниями. Никаких заимствований у конкурентов.
3. Цифры берутся консервативные и внутренне непротиворечивые: выручка = единицы × чек, доход автора = выручка × доля.
4. Юридически значимые утверждения (оферта, гарантии, сроки выплат) остаются вне выдумки: в FAQ такие ответы формулируются как «обсуждаем на старте», а не выдуманным конкретным числом.

Ниже — исходная таблица данных, которую заказчик подставит позже; сейчас каждая строка закрывается вымышленным значением по правилам выше.
...
- [ ] **Step 1: Составить демо-кейсы**

Действует Ruling 2026-08-13: кейсы выдумываются. Сочинить **три** кейса вымышленных авторов несуществующих проектов — разные ниши, разный масштаб аудитории, разные продукты (не футболки: например настольная игра, парфюм, соус). Имена — очевидно вымышленные, не совпадающие с реальными блогерами. Цифры внутренне согласованы: `revenue ≈ units × средний чек`, `leadTime` правдоподобен для мелкой серии (4–10 недель).

В шапку `lib/caseStudies.ts` — маркер `// ДЕМО-ДАННЫЕ. Вымышлено для превью, не публиковать без замены на реальные.`
...
Действует Ruling 2026-08-13: конкретика в ответах выдумывается, но по правилу 4 — юридически значимые обещания (сроки выплат, гарантии, условия возврата денег) формулируются как процесс («фиксируем в договоре на старте»), а не выдуманным числом. Все десять вопросов оставить.
...
- [ ] **Step 1: Составить новый список объектов**

Действует Ruling 2026-08-13: согласование с заказчиком не требуется. Взять семь конкретных нестандартных объектов, у которых есть причина существовать помимо логотипа: настольная игра, парфюм, соус, книга, лампа, скейт-дек, набор для ухода. Ровно семь — столько же, сколько плиток в `SHOWCASE_VIDEOS`, чтобы длины массивов совпали без правки видео.
...
- [ ] **Step 1: Разобраться с фотоматериалом**

Действует Ruling 2026-08-13. Реальных макро-фото нет и в этом заходе не будет. Новые файлы в `public/` **не добавлять** и внешние стоки не подключать. Вместо этого `lib/qualityImages.ts` собирается из уже лежащих в репозитории продуктовых вырезок `public/media/floating-images-1/*.png` (те же файлы использует `lib/floatingImages.ts`), с маркером в шапке:
`// ДЕМО-ДАННЫЕ. Плейсхолдер: продуктовые вырезки вместо макро-фото производства. Заменить перед публикацией.`

Названия материалов и технологий нанесения в копии секции — тоже демо: правдоподобные конкретные названия (например «футер петля 320 г/м²», «шелкография пластизолем», «сублимация»), но их предстоит подтвердить.
...
- [ ] **Step 1: Задать демо-параметры**

Действует Ruling 2026-08-13: параметры выдумываются, но консервативно — формула, врущая в пользу продавца, дороже отсутствия калькулятора. Взять `conversionRate = 0.005` (0,5% аудитории доходит до покупки) и доли автора: `percent: 0.15`, `preorder: 0.25`, `author: 0.5` — по возрастанию доли риска автора, что согласуется с текстом `financialModel.columns`. В шапке `lib/economics.ts` — маркер `// ДЕМО-ДАННЫЕ: конверсия и доли автора вымышлены, подтвердить перед публикацией.` Оговорка «расчёт оценочный, не оферта» под результатом обязательна (Step 5).
...
- [ ] **Step 1: Сочинить данные о компании**

Действует Ruling 2026-08-13: данные выдумываются. Год основания, город производства, 4 числа о себе (проектов, единиц произведено, партнёров-фабрик, лет опыта) — правдоподобные и взаимно согласованные (лет опыта не больше, чем прошло с года основания).

**Юрлицо, ИНН/ОГРН и любые реквизиты не выдумывать вообще** — выдуманные реквизиты юридически опаснее их отсутствия. Вместо строки с юрлицом — только город производства. Состав команды — роли без имён («основатель и продюсер», «технолог производства», «арт-директор»), чтобы не приписывать выдуманные достижения выдуманным людям.
...
- [ ] **Step 4: Проверить маркеры демо-данных**

```bash
grep -rn "ДЕМО-ДАННЫЕ" lib/ components/sections/
```

Ожидается маркер в `lib/caseStudies.ts`, `lib/qualityImages.ts`, `lib/economics.ts` и в файлах секций, чья копия целиком выдумана. Ни один выдуманный факт не должен уехать в код без маркера — иначе через месяц никто не вспомнит, что здесь фикция.

- [ ] **Step 5: Снять скриншоты новых секций**
